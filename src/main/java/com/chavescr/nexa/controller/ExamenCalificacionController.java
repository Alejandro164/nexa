...
@Controller
@RequestMapping("/gestion-academica/examenes/evaluacion")
public class ExamenCalificacionController {

    private final ExamenService service;
    private final HistorialCambioService historialService;

    public ExamenCalificacionController(ExamenService service, HistorialCambioService historialService) {
        this.service = service;
        this.historialService = historialService;
    }

    @GetMapping("/modal")
    public String modal(@RequestParam Long nivelId, @RequestParam Long materiaId, @RequestParam Long examenId,
    ...
    public String guardarLote(@RequestParam Long nivelId, @RequestParam Long materiaId, @RequestParam Long examenId,
        ...
        if (estudianteId != null) {
            for (int i = 0; i < estudianteId.size(); i++) {
                ...
                try {
                    ...
                    FilaNotaExamen fila = service.registrarCalificacion(institucionId, examenId, estudianteId.get(i),
                            puntos, obs);
        ...
        if (!calificados.isEmpty()) {
            String examenTitulo = service.obtenerExamen(institucionId, examenId).getTitulo();
            historialService.registrar(institucionId, nivelId, materiaId, ModuloAcademico.EXAMEN, examenId,
                    examenTitulo, AccionHistorial.CALIFICAR, usuario != null ? usuario.getId() : null,
                    usuario != null ? usuario.getNombre() : null, String.join(", ", calificados));
        }
        String examenCalificado = "\"examenCalificado\":{\"nivelId\":" + nivelId + ",\"materiaId\":" + materiaId + "}";
        if (!errores.isEmpty()) {
