...
public interface NotaConductaRepository extends JpaRepository<NotaConducta, Long> {
    ...
    @Query("SELECT n.estudiante.id, n FROM NotaConducta n WHERE n.direccion.id = :direccionId "
    ...
    Optional<NotaConducta> findByDireccionIdAndPeriodoIdAndEstudianteId(
            Long direccionId, Long periodoId, Long estudianteId);

    @Query("SELECT n.estudiante.id FROM NotaConducta n WHERE n.direccion.id = :direccionId "
            + "AND n.periodo.id = :periodoId AND n.enviada = true AND n.estudiante.id IN :estudianteIds")
    List<Long> findEstudianteIdsEnviados(@Param("direccionId") Long direccionId,
            @Param("periodoId") Long periodoId, @Param("estudianteIds") Collection<Long> estudianteIds);
}
